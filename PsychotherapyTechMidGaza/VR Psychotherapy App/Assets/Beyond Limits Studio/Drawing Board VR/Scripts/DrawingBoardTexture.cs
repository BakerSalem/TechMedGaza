        ...
        public class DrawingBoardTexture : MonoBehaviour, DrawingBoard
        {
            ...
            public void CreateEmptyBoard()
            {
                if (renderTexture == null)
                {
                    ...
                    renderTexture = new RenderTexture(resolutionX, resolutionY, 32, RenderTextureFormat.ARGBFloat);
            ...
            public bool SaveImage(string path, string name)
            {
                ...
                {
                    Debug.LogWarning("fileName cannot be empty!");
                    return false;
                }
...
/*#if UNITY_ANDROID
                NativeGallery.SaveImageToGallery(bytes, "Drawing Board", (name + ".png"), (success, path) => Debug.Log("Media save result: " + success + " " + path));
#else
                if (string.IsNullOrEmpty(path))
                {
                    Debug.LogWarning("FilePath cannot be empty!");
                    return false;
                }
